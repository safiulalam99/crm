...
export const validationSchema = Yup.object().shape({
  ...
  bankdetailsid: Yup.string().nullable(),
  ...
  products: Yup.array()
    .of(
      Yup.object().shape({
        ...
        productlot: Yup.string(),
        languageversion: Yup.string(),
        unitPrice: Yup.number()
          .min(0, 'Unit price must be 0 or more')
          .required('Unit price is required'),
        unitVat: Yup.number()
          .min(0, 'Unit VAT must be 0 or more')
          .required('Unit VAT is required'),
        unitTotal: Yup.number()
          .min(0, 'Unit total must be 0 or more')
          .required('Unit total is required')
      })
    )
    .required('At least one product is required'),
  taxRate: Yup.number()
    .min(0, 'Tax rate must be between 0 and 100')
    .max(100, 'Tax rate must be between 1 and 100'),
